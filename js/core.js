function addEquation()
{
	...
	select_elt.setAttribute("style","margin-right: 3px; display: inline; width: 50px;");
...
{
	var text = "Solution=("+intersections[solutionIndex][0]+" ; "+intersections[solutionIndex][1]+")";
	...
	var z = intersections[solutionIndex][0]*a + intersections[solutionIndex][1]*b;
	z = round100(z);
	var textZ = "Z="+z;
	var layout = {
			annotations: [{
	...
	setTimeout(function(){Plotly.update('drawArea',{},layout);},1000);
}
...
function filterIntersectionPts(dataMatrix,interPts,operations)
{
	...
	for(i = 0; i < intersections.length; i++)
	{
		...
		{
			var res = (dataMatrix[j][0]*intersections[i][0])+(dataMatrix[j][1]*intersections[i][1]); // Getting Y value
			res = round(res);
			switch (ops[j+1]) {
			case "inf_Eg":
				if(res > dataMatrix[j][2]) //Negation
				{
					keepInterPt = false;
				}
				if(dataMatrix[j][0] === 0) //For Horizontal Line
				{
				...
				if(dataMatrix[j][1] === 0) //For Vertical Line
				{
...
{
	return (dataMatrix[index][2] - dataMatrix[index][0]*(x1))/dataMatrix[index][1];
}
...
function getAxisLimits(intersectionPts,margin) //Return [X1max, X2max, X1min, X2min]
{
...
function getIntersectionPts(dataMatrix)
{
	...
	for(i = 0; i < newDataMatrix.length; i++) //Find each X2 for X1(min,max)
	{
	...
	{
		var j;
		for(j = i+1; j < nbCouples; j++) // Getting possible Couples ([i][j])
		{
			// INTERSECTIONS FOR POSSIBLE COUPLE COMBINATIONS
			temp = math.intersect([x1Min,allX2ForX1MinMax[i][0]],[x1Max,allX2ForX1MinMax[i][1]],
				[x1Min,allX2ForX1MinMax[j][0]],[x1Max,allX2ForX1MinMax[j][1]]);
			if(temp != null)
			{
	...
	for(i = 0; i < nbCouples; i++)
	{
		...
		temp = math.intersect([x1Min,allX2ForX1MinMax[i][0]],[x1Max,allX2ForX1MinMax[i][1]],
			[0,allX2ForX1MinMax[i][0]],[0,allX2ForX1MinMax[i][1]]);
		if(temp != null)
		{
			temp[0] = Math.round(temp[0]*100)/100;
			temp[1] = Math.round(temp[1]*100)/100;
	...
	for(i = 0; i < horizontalEquations.length; i++)
	{
		...
		{
			temp = math.intersect([x1Min,horizontalEquations[i]],[x1Max,horizontalEquations[i]],
				[x1Min,allX2ForX1MinMax[j][0]],[x1Max,allX2ForX1MinMax[j][1]]);
			if(temp != null)
			{
				temp[0] = Math.round(temp[0]*100)/100;
	...
	for(i = 0; i < verticalEquations.length; i++)
	{
		for(var j = 0; j < nbCouples; j++)
		{
			temp = math.intersect([verticalEquations[i],-20],[verticalEquations[i],20],
				[x1Min,allX2ForX1MinMax[j][0]],[x1Max,allX2ForX1MinMax[j][1]]);
			if(temp != null)
			{
				temp[0] = Math.round(temp[0]*100)/100;
		...
		if(!duplicate) //Si les intersections n'ont pas encore été définies dans la boucle des axes horizontales
		{
...
function getIntersectionPtsLimits(dataMatrix,axisLimits) // Intersections With Axis Limits
{
	var intersections = [];
	var newDataMatrix = [];
	...
	for(var i = 0; i < nbCouples; i++)
	{
		...
		for(var j = 0; j < len; j++) // Getting possible Couples ([i][j])
		{
			// INTERSECTIONS FOR COUPLES WITH AXIS LIMITS (CLOCKWISE)
			temp = math.intersect([x1Min,allX2ForX1MinMax[i][0]],[x1Max,allX2ForX1MinMax[i][1]],
				axisLimitLines[j],axisLimitLines[j+1]);
			if(temp != null)
			{
				temp[0] = Math.round(temp[0]*100)/100;
				temp[1] = Math.round(temp[1]*100)/100;
...
function getEndpoints(dataMatrix,axisLimits) // Get Extremités des Droites
{
	var endpoints = [];
	var couple = [];
	var i;
	...
	{
		if(dataMatrix[i][1] == 0) //Vertical (b = 0)
		{
		...
		if(dataMatrix[i][0] == 0) //Horizontal (a = 0)
		{
...
function getFinalSolutionLineMax(maximizerValues,intersections,axisLimits) //Maximisation
{
	...
	var cMax = 0;
	var cMin = 0;
	if(maximizerValues[1] < 0) // Si b < 0 => c = cMin
	{
		cMin = maximizerValues[0]*intersections[0][0] + maximizerValues[1]*intersections[0][1]; //Init cMin

		for(var i = 0; i < intersections.length; i++)
		{
			c = maximizerValues[0]*intersections[i][0] + maximizerValues[1]*intersections[i][1];
			if( c < cMin) // Finding c Max
			{
				cMin = c;
				solutionIndex = i;
			}
	...
	for(var i = 0; i < intersections.length; i++) // Si b > 0 => cMax
	{
		c = maximizerValues[0]*intersections[i][0] + maximizerValues[1]*intersections[i][1];
		if( c > cMax) // Finding c Max
		{
			cMax = c;
			solutionIndex = i;
		}
...
function getFinalSolutionLineMin(maximizerValues,intersections,axisLimits) //Minimisation
{
	var solutions = []; //Coordinates of the Final Solution Limiter
	var solutionIndex = 0;
	var temp = 0;
...
function getMaximizerValues() //Maximisation
{
	var values = [];
...
function getMaximizerPlotData(axisLimits,maximizerValues) //Maximisation Initial Maximizer Line Position
{
	//X2 = (-a*X1/b)
...
function getMinimizerPlotData(axisLimits,maximizerValues) //Minimisation Initial Minimizer Line Position
{
	var c;
	var signe = maximizerValues[0]*maximizerValues[1];
	var x2ForX1Max;
	var x2ForX1Min;
	if(signe > 0) // Line start in the NE Corner
	{
		c = maximizerValues[0]*(axisLimits[0] - (axisLimits[0]*10/100))
	...
	c = maximizerValues[0]*(axisLimits[2] - (axisLimits[2]*10/100))
	+ maximizerValues[1]*(axisLimits[1] - (axisLimits[1]*10/100)); // Limits - 10% margin (c=aX1 + aX2)
	//X2 = (c-a*X1/b)
	x2ForX1Min = (c-maximizerValues[0]*axisLimits[2])/maximizerValues[1];
	x2ForX1Max = (c-maximizerValues[0]*axisLimits[0])/maximizerValues[1];
...
function getPolygonPath(intersectionPts)
{
	...
	while(i < nbInterPts)
	{
		if(j < nbInterPts)
		{
			...
			{
				if(!isNaN(intersectionPts[j][0]) || !isNaN(intersectionPts[j][1])) // To avoid NaN Coordinates
				{
			...
			if(intersectionPts[i][1] > intersectionPts[j][1])
			{
				if(!isNaN(intersectionPts[j][0]) || !isNaN(intersectionPts[j][1])) // To avoid NaN Coordinates
				{
					if(j+2 >= nbInterPts) //Lorsqu'on est arrivé à l'extremité droite de la figure
					{
	...
	for(i = 0; i < fullPathArray.length; i++)
	{
		...
		pathString = pathString+""+fullPathArray[i][0]+" "+fullPathArray[i][1]+" "+endChar;
	}
	return pathString;
}
...
function removeEquation(elem)
{
	...
	while(i < numChildren)
	{
		...
		{
			elem.parentNode.parentNode.removeChild(elem.parentNode.parentNode.children[i]);
			break;
		}
		i++;
	}
...
function removeDuplicate(arr)
{
	...
	for(var i = 0; i < arr.length; i++)
	{
		...
		for(var j = 0; j < arr.length; j++)
		{
			...
			if(j != i)
			{
				if(arr[i][1] === arr[j][1] && arr[i][0] === arr[j][0]) // Array i == Array j
				{
					ignored.push(j);
				}
			}
...
function resolve(zoom) // Zoom 0 to 100%
{
	...
	if(verified)
	{
		...
		console.log("ALL INTERSECTIONS");console.log(intersections);

		intersections = getIntersectionPtsPositive(intersections);

		console.log("POSITIVE INTERSECTIONS");console.log(intersections);

		var filteredInterPts = filterIntersectionPts(matrix,intersections,operations);

		if(filteredInterPts.length === 0) // PAS de Solutions
		{
			displaySolutionNone();
			return -1;
		}

		console.log("FILTERED INTERSECTIONS");console.log(filteredInterPts);

		var axisLimits = getAxisLimits(filteredInterPts,axisMargin);

		console.log("LIMITS OF AXIS");console.log(axisLimits); //[maxX,maxY,minX,minY]
		...
		all_intersections = filterIntersectionPts(matrix,all_intersections, operations);
		all_intersections = removeDuplicate(all_intersections);
		all_intersections = getIntersectionPtsPositive(all_intersections);

		console.log("ALL INTERSECTIONS (FILTERED)");console.log(all_intersections);

		var endpoints = getEndpoints(matrix, axisLimits);

		console.log("LINES ENDPOINTS");console.log(endpoints);
		...
		console.log("MAXIMIZER/MINIMIZER VALUES");console.log(maximizerValues);

		var pathString = getPolygonPath(all_intersections);

		console.log("PATH STRING");console.log(pathString);
		...
		if(mode === "max")
		{
			...
			console.log("LIMITER PLOT DATA (MAX)");console.log(limiterPlotData);
			solutionLine = getFinalSolutionLineMax(maximizerValues, filteredInterPts, axisLimits);
			solutionLineAll = getFinalSolutionLineMax(maximizerValues, all_intersections, axisLimits);
			isUniqueSolution = compareSolutions(solutionLine, solutionLineAll);
			console.log("SOLUTION");console.log(solutionLine);
			console.log("SOLUTION (INCL AXISLimits INT)"); console.log(solutionLineAll);
			console.log("UNIQUE SOLUTION ?"); console.log(isUniqueSolution);
		}
		if(mode === "min")
		{
			limiterPlotData = getMinimizerPlotData(axisLimits, maximizerValues);
			console.log("LIMITER PLOT DATA (MIN)");console.log(limiterPlotData);
			solutionLine = getFinalSolutionLineMin(maximizerValues, filteredInterPts, axisLimits);
			solutionLineAll = getFinalSolutionLineMin(maximizerValues, all_intersections, axisLimits);
			isUniqueSolution = compareSolutions(solutionLine, solutionLineAll);
			console.log("SOLUTION");console.log(solutionLine);
			console.log("SOLUTION (INCL AXISLimits INT)"); console.log(solutionLineAll);
			console.log("UNIQUE SOLUTION ?"); console.log(isUniqueSolution);
		...
		var layout = {
		    ...
		    showlegend: true,
		    xaxis: {range: [axisLimits[2],axisLimits[0]],
		    	showgrid: true,
		    	zeroline: true,
		    	fixedrange: true},
	  		yaxis: {range: [axisLimits[3],axisLimits[1]],
	  			showgrid: true,
	  			zeroline: true,
		    	fixedrange: true},
	  		shapes: [{
...
function sortNumberASC(a,b) //Read Array.sort() method documentation
{
	if (a[0] === b[0]) {
        return 0;
    }
...
function verifyInputs()
{
	...
	for(var i = 0; i < limiterFields.length; i++)
	{
		if(limiterFields[i].value == "") {
			...
			limiterFields[i].setAttribute("style","width:20%; border: 1px solid red");
			continue;
		}
		limiterFields[i].setAttribute("style","width:20%");
	}
	if(emptyFields.length > 0 || limiterEmptyFields > 0) {
		$('#alert_text').html("<strong>Veuillez remplir les champs vides</strong><br><br>");
		$('#popup_container').show();
		return false;
	}
	return true;
